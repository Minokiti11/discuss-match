...
export function TopicThreadClient({
  topicId,
  searchParams,
}: {
  topicId: string;
  ...
  const roomId = getParam(searchParams.roomId) || "default";
  const topic = getParam(searchParams.topic);
  const stance = getParam(searchParams.stance);
  const subtopic = getParam(searchParams.subtopic);
